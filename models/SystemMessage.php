...
class SystemMessage extends \yii\db\ActiveRecord
{
     ...
     * @param string $type
     * @param mixed $data
     * @param int $messageInterval timestamp between messages
     * @return bool
     */
    public static function send($title, $message, $type = null, $data = null, $messageInterval = null)
    {
        //Add only 1 hour
        if (SystemMessage::find()->
            where([
                    'and',
                    ['title' => $title],
                    ['>', 'created_at', $messageInterval ?: strtotime('-1 hour')]
                ])->count() > 0
        ) {
            return true;
        }
